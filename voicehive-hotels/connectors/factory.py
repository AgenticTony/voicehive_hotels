...
from connectors.utils.vault_client import get_vault_client, VaultError, DevelopmentVaultClient
...
class ConnectorRegistry:
    ...
    def _load_capability_matrix(self):
        ...
        if matrix_path.exists():
            with open(matrix_path, 'r') as f:
                ...
                logger.info(f"Loaded capability matrix with {len(self._capability_matrix.get('vendors', {}))} vendors")
        else:
                ...
                try:
                    ...
                    for name, obj in inspect.getmembers(module):
                        if (inspect.isclass(obj) and 
                            ...
                            logger.info(f"Discovered connector: {vendor_name} ({obj.__name__})")
                            
                except Exception as e:
                    logger.error(f"Failed to load connector from {vendor_dir.name}: {e}")
    ...
    def register(self, vendor: str, connector_class: Type[PMSConnector], 
    ...
    def list_vendors(self, status: Optional[ConnectorStatus] = None) -> List[str]:
        ...
        if status:
            return [
                vendor for vendor, meta in self._metadata.items() 
...
class ConnectorFactory:
    ...
    def __init__(self, registry: Optional[ConnectorRegistry] = None, use_vault: bool = True):
        ...
        if self.use_vault:
            try:
                ...
                logger.warning(f"Failed to initialize Vault client: {e}. Falling back to config-based credentials.")
    ...
    def _prepare_config(self, vendor: str, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Successfully loaded credentials from Vault for {vendor}/{hotel_id}")
            
        except VaultError as e:
            # Log warning but continue with config-based credentials
            logger.warning(f"Failed to fetch credentials from Vault: {e}. Using config-based credentials.")
        except Exception as e:
...
def register_connector(vendor: str, connector_class: Type[PMSConnector], 
                      metadata: Optional[ConnectorMetadata] = None):
