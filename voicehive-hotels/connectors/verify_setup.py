...
def verify_mock_connector():
    ...
    try:
        from connectors import register_connector, get_connector, ConnectorMetadata, ConnectorStatus
        from connectors.contracts import MockConnector
        
        # Register mock
        register_connector("test_mock", MockConnector, ConnectorMetadata(
            vendor="test_mock",
            name="Test Mock",
            version="1.0.0",
...
def verify_apaleo_discovery():
    ...
    try:
        ...
        if "apaleo" in vendors:
            ...
            print(f"  Capabilities: {sum(1 for v in metadata.capabilities.values() if v)} supported")
            return True
        else:
