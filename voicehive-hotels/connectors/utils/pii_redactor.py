...
class PIIRedactor:
    ...
    CONFIRMATION_NUMBER_PATTERN = r'\b(confirmation|conf|booking|res)\s*#?\s*([A-Z0-9]{6,12})\b'
    ...
    def redact_dict(self, data: Dict[str, Any], 
        ...
        default_sensitive = [
            "email", "phone", "password", "credit_card", "card_number",
            "cvv", "ssn", "passport", "id_number", "api_key", "secret"
        ]
    ...
    def _apply_redaction(self, text: str, start: int, end: int) -> str:
        ...
        if end - start > 2:
            return text[:start+1] + self.redact_char * (end-start-2) + text[end-1:]
        else:
    ...
    def _redact_custom_patterns(self, text: str) -> str:
        ...
        # Room numbers
        text = re.sub(self.ROOM_NUMBER_PATTERN, r'\1 <ROOM_NUMBER>', text, flags=re.IGNORECASE)
        
        # Confirmation numbers  
        text = re.sub(self.CONFIRMATION_NUMBER_PATTERN, r'\1 <CONFIRMATION>', text, flags=re.IGNORECASE)
        
        # Guest/Member IDs
        text = re.sub(self.GUEST_ID_PATTERN, r'\1 <GUEST_ID>', text, flags=re.IGNORECASE)
