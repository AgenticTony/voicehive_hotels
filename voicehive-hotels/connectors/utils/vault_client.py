...
class VaultClient:
    ...
    def _authenticate_kubernetes(self):
        ...
        try:
            ...
            logger.info(f"Authenticated with Vault using Kubernetes role: {self.kubernetes_role}")
...
class AsyncVaultClient:
    ...
    async def read_pms_credentials(self, vendor: str, hotel_id: str) -> Dict[str, Any]:
        ...
        return await loop.run_in_executor(
            self._executor,
            self._client.read_pms_credentials,
            vendor,
            hotel_id
        )
